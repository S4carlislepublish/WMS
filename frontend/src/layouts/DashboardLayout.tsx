...
import { ChatBubbleLeftRightIcon, XMarkIcon, Bars3Icon } from "@heroicons/react/24/outline";
...
const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    if (birthdayEmployees.length > 0 && !sessionStorage.getItem("birthday_popup_shown")) {
      ...
      setTimeout(() => { setBirthdayModal(false); setShowPopup(true); }, 5000);
    } else {
      setShowPopup(true);
    }
  }, [birthdayEmployees]);
  ...
  useEffect(() => {
    const loadReportingEmployees = async () => {
      ...
      const response = await fetch(`${BASE_URL}/employees/reporting-employees/${uid}`);
  ...
  const viewAttendance = async (emp: any) => {
    try {
      const response = await fetch(`${BASE_URL}/attendance/details/${emp.employee_id}`);
      const data = await response.json();
  ...
  const fetchMessages = async () => {
    try {
      ...
      const response = await fetch(`${BASE_URL}/communications/employee/${employeeId}`);
...
handleSelectUser(
  ...
  const sendAnnouncement = async () => {
    ...
    try {
      ...
      setOfficeText("");
      loadAnnouncements();
    } catch (error) { console.error("Send Announcement Error:", error); }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-900">
      ...
      <div className="flex items-center justify-between border-b border-gray-700 bg-gray-800 px-4 py-3 lg:hidden">
        <button onClick={() => setSidebarOpen(!sidebarOpen)} className="rounded-lg p-2 text-white transition hover:bg-gray-700">
