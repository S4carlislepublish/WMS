...
import {
  HomeIcon, CheckCircleIcon, ClockIcon, CalendarDaysIcon,
  ChartBarIcon, UserCircleIcon, SparklesIcon,
...
const [shiftDate, setShiftDate] =
  ...
  const isMyBirthday = birthdayEmployees.some((emp: any) => Number(emp.user_id) === Number(user?.id));

  const managerName = `${currentEmployee?.first_name} ${currentEmployee?.last_name}`.trim().toLowerCase();
  const approvalLeaves = leaveRequests.filter((leave: any) =>
    leave.reporting_manager?.trim().toLowerCase() === managerName
  );
  const totalBalance = (currentEmployee?.sick_leave || 0) + (currentEmployee?.casual_leave || 0) + (currentEmployee?.earned_leave || 0);
  const pendingLeaveCount = leaveRequests.filter((leave: any) => leave.status === "Pending").length;
  ...
  const fetchTodayBirthdays = async () => {
    ...
    } catch (err) { setBirthdayEmployees([]); }
  };
  ...
  const loadShiftRequests = async () => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/shifts/employee/${currentEmployee.user_id}`);
      const data = await res.json();
  ...
  const handleCheckIn = async () => {
    try {
      ...
      if (!userId) { showPopup("error", "User Not Found", "Unable to identify current user."); return; }
      ...
      if (!response.ok) { showPopup("error", "Check In Failed", data.message || data.error || "Check In Failed"); return; }
      showPopup("success", "Check In Successful", data.message || "You have checked in successfully.");
      setTimeout(() => window.location.reload(), 1000);
    } catch (error) { showPopup("error", "Check In Error", "Something went wrong while checking in."); }
  };

  const handleCheckOut = async () => {
    try {
      if (isLunchBreak) { showPopup("warning", "Lunch Break Active", "Please stop lunch break before checkout."); return; }
      if (isTeaBreak) { showPopup("warning", "Tea Break Active", "Please stop tea break before checkout."); return; }
      const userId = localStorage.getItem("user_id");
      ...
      const attendanceResponse = await fetch(`${BASE_URL}/attendance/history/${userId}`);
      ...
      showPopup("success", "Check Out Successful", "You have checked out successfully.");
    } catch (error) { alert("Something went wrong while checking out."); }
  };

  const handleLunchBreak = async () => {
    if (!isCheckedIn) { showPopup("error", "Check-In Required", "Please check in before starting Lunch Break."); return; }
    try {
      const userId = localStorage.getItem("user_id");
      if (!userId) return;
      if (!isLunchBreak) {
        ...
        if (lunchStartTime) {
          const seconds = Math.floor((new Date().getTime() - lunchStartTime.getTime()) / 1000);
          setTotalLunchSeconds((prev) => prev + seconds);
          setIsLunchBreak(false);
          setLunchStartTime(null);
          await fetch(`${BASE_URL}/attendance/lunch-break`, {
            method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ user_id: Number(userId), action: "stop", break_seconds: seconds }),
          });
        }
      }
    } catch (error) { showPopup("error", "Error", "Something went wrong."); }
  };

  const handleTeaBreak = async () => {
    if (!isCheckedIn) { showPopup("error", "Check-In Required", "Please check in before starting Tea Break."); return; }
    try {
      const userId = localStorage.getItem("user_id");
      if (!userId) return;
      if (!isTeaBreak) {
        ...
        const seconds = Math.floor((new Date().getTime() - teaStartTime.getTime()) / 1000);
    ...
    } catch (error) { showPopup("error", "Error", "Something went wrong while handling Tea Break."); }
  };

  // --- Leave Handlers ---
  const handleLeaveSubmit = async (e: React.FormEvent, leaveForm: any, editingLeave: any) => {
    e.preventDefault();
    try {
      let response;
      if (editingLeave) {
        response = await fetch(`${BASE_URL}/leaves/update/${editingLeave.id}`, {
          ...
          body: JSON.stringify({ leave_type: leaveForm.leaveType, from_date: leaveForm.fromDate, to_date: leaveForm.toDate, total_days: leaveForm.totalDays, handover_to: leaveForm.handoverTo, emergency_contact: leaveForm.emergencyContact, reason: leaveForm.reason }),
        });
      } else {
        response = await fetch(`${BASE_URL}/leaves/`, {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ employee_id: currentEmployee?.id, employee_name: `${currentEmployee?.first_name} ${currentEmployee?.last_name}`, leave_type: leaveForm.leaveType, from_date: leaveForm.fromDate, to_date: leaveForm.toDate, total_days: leaveForm.totalDays, reporting_manager: currentEmployee?.reporting_manager, handover_to: leaveForm.handoverTo, emergency_contact: leaveForm.emergencyContact, reason: leaveForm.reason }),
        });
      }
      const data = await response.json();
      if (data.success) {
  ...
  const approveLeave = async (id: number) => {
    try {
      const res = await fetch(`${BASE_URL}/leaves/approve/${id}`, { method: "PUT" });
      const data = await res.json();
      if (data.success) { toast.success("Leave Approved"); loadLeaves(); }
    } catch (err) { console.log(err); }
  };

  const rejectLeave = async (id: number) => {
    try {
      const res = await fetch(`${BASE_URL}/leaves/reject/${id}`, { method: "PUT" });
      const data = await res.json();
      if (data.success) { toast.success("Leave Rejected"); loadLeaves(); }
    } catch (err) { console.log(err); }
  };

  const cancelLeave = async (id: number) => {
    try {
      const res = await fetch(`${BASE_URL}/leaves/cancel/${id}`, { method: "PUT" });
      const data = await res.json();
      if (data.success) { toast.success("Leave Cancelled"); loadLeaves(); }
    } catch (err) { console.log(err); }
  };
  ...
  const submitShiftRequest = async (shiftForm: any) => {
    try {
      if (!currentEmployee) { toast.error("Employee details not found"); return; }
  ...
  reason: shiftForm.reason
      ...
      const data = await response.json();
      if (!response.ok) { toast.error(data.message || "Failed to submit shift request"); return; }
      if (data.success) { toast.success("Shift Request Submitted Successfully"); loadShiftRequests(); }
      else { toast.error(data.message || "Request Failed"); }
    } catch (error) { toast.error("Server Error"); }
  };

  const approveShift = async (id: number) => {
    const response = await fetch(`${BASE_URL}/shifts/approve/${id}`, { method: "PUT" });
    const data = await response.json();
    if (data.success) { toast.success("Shift Approved Successfully"); loadShiftRequests(); loadManagerShiftRequests(); }
  };

  const rejectShift = async (id: number) => {
    const response = await fetch(`${BASE_URL}/shifts/reject/${id}`, { method: "PUT" });
    const data = await response.json();
    if (data.success) { toast.success("Shift Rejected"); loadShiftRequests(); loadManagerShiftRequests(); }
  };
  ...
  const sendBirthdayWish = async (emp: any) => {
    ...
    await fetch(`${BASE_URL}/communications`, {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ employee_id: emp.id, employee_name: `${emp.first_name} ${emp.last_name}`, receiver_id: emp.user_id, message_type: "employee", created_by: senderName, message: `🎂 Happy Birthday ${emp.first_name}! Wishing you happiness, success and prosperity. 🎉` }),
    });
  };
  ...
  useEffect(() => {
    ...
    if (savedCheckIn) { setIsCheckedIn(true); setCheckInTime(new Date(savedCheckIn)); }
  }, []);
  ...
  useEffect(() => {
    if (birthdayEmployees.length > 0 && !sessionStorage.getItem("birthday_popup_shown")) {
  ...
  useEffect(() => {
    ...
    if (isCheckedIn && checkInTime && !isLunchBreak && !isTeaBreak) {
      interval = setInterval(() => {
        const totalWorkedSeconds = Math.floor((new Date().getTime() - checkInTime.getTime()) / 1000);
        const workingSeconds = totalWorkedSeconds - totalLunchSeconds - totalTeaSeconds;
  ...
  }, [isCheckedIn, checkInTime, isLunchBreak, isTeaBreak, totalLunchSeconds, totalTeaSeconds]);

  useEffect(() => {
    if (activeTab === "shift") { loadShiftRequests(); loadManagerShiftRequests(); }
  }, [activeTab]);
    ...
    <>
      {confirmModal && (
        <ConfirmModal
          ...
          onConfirm={() => { setConfirmModal(false); handleCheckOut(); }}
        />
      )}

      <PopupModal popup={popup} onClose={() => setPopup({ ...popup, show: false })} />
      ...
      <div className="min-h-screen bg-gray-100">
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <motion.div key={activeTab} variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
            {activeTab === 'overview' && (
              <OverviewTab
