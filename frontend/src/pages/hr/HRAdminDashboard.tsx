...
import { theme, NAV, DEFAULT_NEW_EMP, DEFAULT_PROFILE_DATA } from "./data/hrMockData";
...
export default function HRAdminDashboard() {
  ...
  const fetchLeaveRequests = async () => {
    try {
      ...
      const formatted = data.map((leave: any) => ({
        ...
        av: leave.employee_name?.split(" ")?.map((n: string) => n[0])?.join("")?.toUpperCase(),
  ...
  const counts = useMemo(() => {
    ...
    const presentEmployeeIds = [...new Set(
      attendance.filter((att) => att.attendance_date === today).map((att) => att.user_id)
    )];
    const activeEmployees = employeeUsers.filter((emp) => presentEmployeeIds.includes(emp.user_id)).length;
  ...
  const filteredEmps = employees.filter((e) =>
    ...
    (e.reporting_manager || "").toLowerCase().includes(search.toLowerCase()) ||
  ...
  const handleApproveLeave = async (id: number) => {
    try {
      const response = await fetch(`${BASE_URL}/leaves/approve/${id}`, { method: "PUT" });
      const data = await response.json();
  ...
  const handleRejectLeave = async (id: number) => {
    try {
      const response = await fetch(`${BASE_URL}/leaves/reject/${id}`, { method: "PUT" });
      const data = await response.json();
      if (data.success) fetchLeaveRequests();
    } catch (error) { console.error(error); }
  };
...
formData.append(
  ...
  const handleProfileComplete = async () => {
    ...
    try {
      const response = await fetch(`${BASE_URL}/employees/${currentEmployee.id}`, {
        method: "PATCH",
    ...
    } catch (error) { console.error(error); alert("Error completing profile"); }
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: theme.bg, fontFamily: "system-ui, -apple-system, sans-serif" }}>
          ...
          {NAV.map((item) => {
            ...
            return (
              <button key={item.id} onClick={() => setNav(item.id)} style={{
                border: "none", background: isActive ? `${theme.accent}18` : "transparent",
                color: isActive ? theme.accent : theme.textMuted,
                padding: "8px 14px", borderRadius: 8,
                display: "flex", alignItems: "center", gap: 8,
                fontSize: 13, fontWeight: isActive ? 700 : 500, cursor: "pointer", whiteSpace: "nowrap",
              }}>
      ...
      <main style={{ padding: 24 }}>
        {nav === "dashboard"   && <DashboardTab counts={counts} employees={employees} />}
